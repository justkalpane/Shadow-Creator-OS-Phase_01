# SKL-PH3B-M-223-PACING_RHYTHM_EDITOR

## 1. Skill Identity
- Skill ID: M-223
- Skill Name: Pacing and Rhythm Editor
- Legacy Alias (Compatibility): Pacing & Rhythm Optimizer
- Alias Names: Pacing & Rhythm Optimizer
...
## 2. Purpose
Optimize video pacing and rhythm with deterministic cut-timing refinements.
...
- Upstream Dependencies: pacing_engine_output (from M-114), editing_sequence (from M-218), audio_specs (from M-220)
- Upstream Skill IDs: M-114, M-218, M-220
- Downstream Consumers: M-224 Title Card Generator
- Downstream Skill IDs: M-224
...
STEP 3: Load deterministic video policy profile for Pacing and Rhythm Editor.
...
STEP 5: Generate primary output payload field rhythm_curve.
...
STEP 8: Assemble typed output packet pacing_optimization_packet.
STEP 9: Append packet to dossier.media_vein.pacing_rhythm_editor and append se_packet_index row.
...
~~~json
{
  "instance_id": "M-223-[timestamp]-[instance]",
  "artifact_family": "pacing_optimization_packet",
  ...
  "payload": {
    "skill_id": "M-223",
    "skill_name": "Pacing and Rhythm Editor",
    "primary_output": {
      "rhythm_curve": "[artifact]"
    },
    "routing": {
      "on_success": "M-224",
      "on_error": "WF-900",
...
## 12. Validation/Done
**Test Cases**
- TEST-PH3B-M-223-001: Valid required inputs produce deterministic pacing_optimization_packet output
- TEST-PH3B-M-223-002: Missing dossier_id routes to WF-900 with explicit validation details
- TEST-PH3B-M-223-003: Missing route_id routes to WF-900 without packet emission
- TEST-PH3B-M-223-004: Missing required upstream packet fails closed and escalates to WF-900
- TEST-PH3B-M-223-005: Lineage references are preserved for all upstream dependencies
- TEST-PH3B-M-223-006: Primary output field rhythm_curve is present and non-empty
- TEST-PH3B-M-223-007: Deterministic logic yields same output structure for identical input state
- TEST-PH3B-M-223-008: Schema validation rejects untyped payload prior to dossier mutation
- TEST-PH3B-M-223-009: Dossier writes target only dossier.media_vein.pacing_rhythm_editor
- TEST-PH3B-M-223-010: Mutation uses append_to_array and never overwrites prior data
- TEST-PH3B-M-223-011: se_packet_index append row includes lineage_reference and instance_id
- TEST-PH3B-M-223-012: Replay branch routes to WF-021 for remodify requests
- TEST-PH3B-M-223-013: Escalation branch routes to WF-900 for validation or policy failures
- TEST-PH3B-M-223-014: Output packet includes timestamp, writer_id, skill_id, instance_id, schema_version
- TEST-PH3B-M-223-015: Forbidden mutation attempt is blocked and audited
- TEST-PH3B-M-223-016: Downstream routing hints are registry-bound and deterministic
- TEST-PH3B-M-223-017: Audit entry includes operation, route_id, source packet, and confidence markers
- TEST-PH3B-M-223-018: Acceptance gate fails closed when governance metadata is incomplete
