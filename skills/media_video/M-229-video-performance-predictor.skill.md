# SKL-PH3B-M-229-VIDEO_PERFORMANCE_PREDICTOR

## 1. Skill Identity
- Skill ID: M-229
- Skill Name: Video Performance Predictor
- Legacy Alias (Compatibility): Video Performance Predictor
- Alias Names: none
...
## 2. Purpose
Predict video performance metrics and risk flags using deterministic scoring frameworks.
...
- Upstream Dependencies: all video specifications (M-216-M-228), historical_performance_data
- Upstream Skill IDs: M-216, M-217, M-218, M-219, M-220, M-221, M-222, M-223, M-224, M-225, M-226, M-227, M-228
- Downstream Consumers: M-230 Platform-Specific Video Adapter
- Downstream Skill IDs: M-230
...
STEP 3: Load deterministic video policy profile for Video Performance Predictor.
...
STEP 5: Generate primary output payload field predicted_engagement_score.
...
STEP 8: Assemble typed output packet performance_prediction_packet.
STEP 9: Append packet to dossier.media_vein.video_performance_predictor and append se_packet_index row.
...
~~~json
{
  "instance_id": "M-229-[timestamp]-[instance]",
  "artifact_family": "performance_prediction_packet",
  ...
  "payload": {
    "skill_id": "M-229",
    "skill_name": "Video Performance Predictor",
    "primary_output": {
      "predicted_engagement_score": "[artifact]"
    },
    "routing": {
      "on_success": "M-230",
      "on_error": "WF-900",
...
## 12. Validation/Done
**Test Cases**
- TEST-PH3B-M-229-001: Valid required inputs produce deterministic performance_prediction_packet output
- TEST-PH3B-M-229-002: Missing dossier_id routes to WF-900 with explicit validation details
- TEST-PH3B-M-229-003: Missing route_id routes to WF-900 without packet emission
- TEST-PH3B-M-229-004: Missing required upstream packet fails closed and escalates to WF-900
- TEST-PH3B-M-229-005: Lineage references are preserved for all upstream dependencies
- TEST-PH3B-M-229-006: Primary output field predicted_engagement_score is present and non-empty
- TEST-PH3B-M-229-007: Deterministic logic yields same output structure for identical input state
- TEST-PH3B-M-229-008: Schema validation rejects untyped payload prior to dossier mutation
- TEST-PH3B-M-229-009: Dossier writes target only dossier.media_vein.video_performance_predictor
- TEST-PH3B-M-229-010: Mutation uses append_to_array and never overwrites prior data
- TEST-PH3B-M-229-011: se_packet_index append row includes lineage_reference and instance_id
- TEST-PH3B-M-229-012: Replay branch routes to WF-021 for remodify requests
- TEST-PH3B-M-229-013: Escalation branch routes to WF-900 for validation or policy failures
- TEST-PH3B-M-229-014: Output packet includes timestamp, writer_id, skill_id, instance_id, schema_version
- TEST-PH3B-M-229-015: Forbidden mutation attempt is blocked and audited
- TEST-PH3B-M-229-016: Downstream routing hints are registry-bound and deterministic
- TEST-PH3B-M-229-017: Audit entry includes operation, route_id, source packet, and confidence markers
- TEST-PH3B-M-229-018: Acceptance gate fails closed when governance metadata is incomplete
