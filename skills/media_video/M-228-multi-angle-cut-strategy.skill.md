# SKL-PH3B-M-228-MULTI_ANGLE_CUT_STRATEGY

## 1. Skill Identity
- Skill ID: M-228
- Skill Name: Multi-Angle Cut Strategy
- Legacy Alias (Compatibility): Multi-Angle Coordination Engine
- Alias Names: Multi-Angle Coordination Engine
...
## 2. Purpose
Coordinate multi-camera and multi-angle cuts with deterministic synchronization rules.
...
- Upstream Dependencies: shot_list (from M-216), editing_sequence (from M-218)
- Upstream Skill IDs: M-216, M-218
- Downstream Consumers: M-229 Video Performance Predictor
- Downstream Skill IDs: M-229
...
STEP 3: Load deterministic video policy profile for Multi-Angle Cut Strategy.
...
STEP 5: Generate primary output payload field camera_assignments.
...
STEP 8: Assemble typed output packet multiangle_packet.
STEP 9: Append packet to dossier.media_vein.multi_angle_cut_strategy and append se_packet_index row.
STEP 10: Emit deterministic routing decision to M-229 or WF-900/WF-021.

## 7. Outputs
- Output Packet Family: multiangle_packet
- JSON Schema Reference: schemas/packets/multiangle_packet.schema.json
- Dossier Write Target: dossier.media_vein.multi_angle_cut_strategy
- se_packet_index Registration: required append row with lineage and audit metadata

~~~json
{
  "instance_id": "M-228-[timestamp]-[instance]",
  "artifact_family": "multiangle_packet",
  ...
  "payload": {
    "skill_id": "M-228",
    "skill_name": "Multi-Angle Cut Strategy",
    "primary_output": {
      "camera_assignments": "[artifact]"
    },
    "routing": {
      "on_success": "M-229",
      "on_error": "WF-900",
...
## 12. Validation/Done
**Test Cases**
- TEST-PH3B-M-228-001: Valid required inputs produce deterministic multiangle_packet output
- TEST-PH3B-M-228-002: Missing dossier_id routes to WF-900 with explicit validation details
- TEST-PH3B-M-228-003: Missing route_id routes to WF-900 without packet emission
- TEST-PH3B-M-228-004: Missing required upstream packet fails closed and escalates to WF-900
- TEST-PH3B-M-228-005: Lineage references are preserved for all upstream dependencies
- TEST-PH3B-M-228-006: Primary output field camera_assignments is present and non-empty
- TEST-PH3B-M-228-007: Deterministic logic yields same output structure for identical input state
- TEST-PH3B-M-228-008: Schema validation rejects untyped payload prior to dossier mutation
- TEST-PH3B-M-228-009: Dossier writes target only dossier.media_vein.multi_angle_cut_strategy
- TEST-PH3B-M-228-010: Mutation uses append_to_array and never overwrites prior data
- TEST-PH3B-M-228-011: se_packet_index append row includes lineage_reference and instance_id
- TEST-PH3B-M-228-012: Replay branch routes to WF-021 for remodify requests
- TEST-PH3B-M-228-013: Escalation branch routes to WF-900 for validation or policy failures
- TEST-PH3B-M-228-014: Output packet includes timestamp, writer_id, skill_id, instance_id, schema_version
- TEST-PH3B-M-228-015: Forbidden mutation attempt is blocked and audited
- TEST-PH3B-M-228-016: Downstream routing hints are registry-bound and deterministic
- TEST-PH3B-M-228-017: Audit entry includes operation, route_id, source packet, and confidence markers
- TEST-PH3B-M-228-018: Acceptance gate fails closed when governance metadata is incomplete
