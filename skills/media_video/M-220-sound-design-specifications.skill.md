# SKL-PH3B-M-220-SOUND_DESIGN_SPECIFICATIONS

## 1. Skill Identity
- Skill ID: M-220
- Skill Name: Sound Design Specifications
- Legacy Alias (Compatibility): Sound Design Brief
- Alias Names: Sound Design Brief
...
## 2. Purpose
Specify sound design palette and cue timing for cinematic coherence.
...
- Upstream Dependencies: pacing_engine_output (from M-114), emotional_arc (from M-106)
- Upstream Skill IDs: M-114, M-106
- Downstream Consumers: M-221 Motion Graphics Storyboard
- Downstream Skill IDs: M-221
...
STEP 3: Load deterministic video policy profile for Sound Design Specifications.
...
STEP 5: Generate primary output payload field sound_palette.
...
STEP 8: Assemble typed output packet sound_design_packet.
STEP 9: Append packet to dossier.media_vein.sound_design_specifications and append se_packet_index row.
STEP 10: Emit deterministic routing decision to M-221 or WF-900/WF-021.

## 7. Outputs
- Output Packet Family: sound_design_packet
- JSON Schema Reference: schemas/packets/sound_design_packet.schema.json
- Dossier Write Target: dossier.media_vein.sound_design_specifications
- se_packet_index Registration: required append row with lineage and audit metadata

~~~json
{
  "instance_id": "M-220-[timestamp]-[instance]",
  "artifact_family": "sound_design_packet",
  ...
  "payload": {
    "skill_id": "M-220",
    "skill_name": "Sound Design Specifications",
    "primary_output": {
      "sound_palette": "[artifact]"
    },
    "routing": {
      "on_success": "M-221",
      "on_error": "WF-900",
...
## 12. Validation/Done
**Test Cases**
- TEST-PH3B-M-220-001: Valid required inputs produce deterministic sound_design_packet output
- TEST-PH3B-M-220-002: Missing dossier_id routes to WF-900 with explicit validation details
- TEST-PH3B-M-220-003: Missing route_id routes to WF-900 without packet emission
- TEST-PH3B-M-220-004: Missing required upstream packet fails closed and escalates to WF-900
- TEST-PH3B-M-220-005: Lineage references are preserved for all upstream dependencies
- TEST-PH3B-M-220-006: Primary output field sound_palette is present and non-empty
- TEST-PH3B-M-220-007: Deterministic logic yields same output structure for identical input state
- TEST-PH3B-M-220-008: Schema validation rejects untyped payload prior to dossier mutation
- TEST-PH3B-M-220-009: Dossier writes target only dossier.media_vein.sound_design_specifications
- TEST-PH3B-M-220-010: Mutation uses append_to_array and never overwrites prior data
- TEST-PH3B-M-220-011: se_packet_index append row includes lineage_reference and instance_id
- TEST-PH3B-M-220-012: Replay branch routes to WF-021 for remodify requests
- TEST-PH3B-M-220-013: Escalation branch routes to WF-900 for validation or policy failures
- TEST-PH3B-M-220-014: Output packet includes timestamp, writer_id, skill_id, instance_id, schema_version
- TEST-PH3B-M-220-015: Forbidden mutation attempt is blocked and audited
- TEST-PH3B-M-220-016: Downstream routing hints are registry-bound and deterministic
- TEST-PH3B-M-220-017: Audit entry includes operation, route_id, source packet, and confidence markers
- TEST-PH3B-M-220-018: Acceptance gate fails closed when governance metadata is incomplete
