# SKL-PH3B-M-222-VIDEO_TRANSITIONS_OPTIMIZER

## 1. Skill Identity
- Skill ID: M-222
- Skill Name: Video Transitions Optimizer
- Legacy Alias (Compatibility): Transition Sequence Builder
- Alias Names: Transition Sequence Builder
...
## 2. Purpose
Design transition effects and transition timing for flow consistency.
...
- Upstream Dependencies: editing_sequence (from M-218), motion_graphics (from M-221)
- Upstream Skill IDs: M-218, M-221
- Downstream Consumers: M-223 Pacing and Rhythm Editor
- Downstream Skill IDs: M-223
...
STEP 3: Load deterministic video policy profile for Video Transitions Optimizer.
...
STEP 5: Generate primary output payload field transition_types.
...
STEP 8: Assemble typed output packet transition_packet.
STEP 9: Append packet to dossier.media_vein.video_transitions_optimizer and append se_packet_index row.
STEP 10: Emit deterministic routing decision to M-223 or WF-900/WF-021.

## 7. Outputs
- Output Packet Family: transition_packet
- JSON Schema Reference: schemas/packets/transition_packet.schema.json
- Dossier Write Target: dossier.media_vein.video_transitions_optimizer
- se_packet_index Registration: required append row with lineage and audit metadata

~~~json
{
  "instance_id": "M-222-[timestamp]-[instance]",
  "artifact_family": "transition_packet",
  ...
  "payload": {
    "skill_id": "M-222",
    "skill_name": "Video Transitions Optimizer",
    "primary_output": {
      "transition_types": "[artifact]"
    },
    "routing": {
      "on_success": "M-223",
      "on_error": "WF-900",
...
## 12. Validation/Done
**Test Cases**
- TEST-PH3B-M-222-001: Valid required inputs produce deterministic transition_packet output
- TEST-PH3B-M-222-002: Missing dossier_id routes to WF-900 with explicit validation details
- TEST-PH3B-M-222-003: Missing route_id routes to WF-900 without packet emission
- TEST-PH3B-M-222-004: Missing required upstream packet fails closed and escalates to WF-900
- TEST-PH3B-M-222-005: Lineage references are preserved for all upstream dependencies
- TEST-PH3B-M-222-006: Primary output field transition_types is present and non-empty
- TEST-PH3B-M-222-007: Deterministic logic yields same output structure for identical input state
- TEST-PH3B-M-222-008: Schema validation rejects untyped payload prior to dossier mutation
- TEST-PH3B-M-222-009: Dossier writes target only dossier.media_vein.video_transitions_optimizer
- TEST-PH3B-M-222-010: Mutation uses append_to_array and never overwrites prior data
- TEST-PH3B-M-222-011: se_packet_index append row includes lineage_reference and instance_id
- TEST-PH3B-M-222-012: Replay branch routes to WF-021 for remodify requests
- TEST-PH3B-M-222-013: Escalation branch routes to WF-900 for validation or policy failures
- TEST-PH3B-M-222-014: Output packet includes timestamp, writer_id, skill_id, instance_id, schema_version
- TEST-PH3B-M-222-015: Forbidden mutation attempt is blocked and audited
- TEST-PH3B-M-222-016: Downstream routing hints are registry-bound and deterministic
- TEST-PH3B-M-222-017: Audit entry includes operation, route_id, source packet, and confidence markers
- TEST-PH3B-M-222-018: Acceptance gate fails closed when governance metadata is incomplete
