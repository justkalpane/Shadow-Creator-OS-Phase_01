# SKL-PH3B-M-230-PLATFORM_SPECIFIC_VIDEO_ADAPTER

## 1. Skill Identity
- Skill ID: M-230
- Skill Name: Platform-Specific Video Adapter
- Legacy Alias (Compatibility): Platform Video Adapter
- Alias Names: Platform Video Adapter
...
## 2. Purpose
Adapt video specifications for platform-specific technical and presentation requirements.
...
- Upstream Dependencies: all video specifications (M-216-M-229)
- Upstream Skill IDs: M-216, M-217, M-218, M-219, M-220, M-221, M-222, M-223, M-224, M-225, M-226, M-227, M-228, M-229
- Downstream Consumers: Phase 3C audio production entry
- Downstream Skill IDs: M-231
...
STEP 3: Load deterministic video policy profile for Platform-Specific Video Adapter.
...
STEP 5: Generate primary output payload field platform_variant_specifications.
...
STEP 8: Assemble typed output packet platform_video_packet.
STEP 9: Append packet to dossier.media_vein.platform_specific_video_adapter and append se_packet_index row.
...
~~~json
{
  "instance_id": "M-230-[timestamp]-[instance]",
  "artifact_family": "platform_video_packet",
  ...
  "payload": {
    "skill_id": "M-230",
    "skill_name": "Platform-Specific Video Adapter",
    "primary_output": {
      "platform_variant_specifications": "[artifact]"
    },
    "routing": {
      "on_success": "M-231",
      "on_error": "WF-900",
...
## 12. Validation/Done
**Test Cases**
- TEST-PH3B-M-230-001: Valid required inputs produce deterministic platform_video_packet output
- TEST-PH3B-M-230-002: Missing dossier_id routes to WF-900 with explicit validation details
- TEST-PH3B-M-230-003: Missing route_id routes to WF-900 without packet emission
- TEST-PH3B-M-230-004: Missing required upstream packet fails closed and escalates to WF-900
- TEST-PH3B-M-230-005: Lineage references are preserved for all upstream dependencies
- TEST-PH3B-M-230-006: Primary output field platform_variant_specifications is present and non-empty
- TEST-PH3B-M-230-007: Deterministic logic yields same output structure for identical input state
- TEST-PH3B-M-230-008: Schema validation rejects untyped payload prior to dossier mutation
- TEST-PH3B-M-230-009: Dossier writes target only dossier.media_vein.platform_specific_video_adapter
- TEST-PH3B-M-230-010: Mutation uses append_to_array and never overwrites prior data
- TEST-PH3B-M-230-011: se_packet_index append row includes lineage_reference and instance_id
- TEST-PH3B-M-230-012: Replay branch routes to WF-021 for remodify requests
- TEST-PH3B-M-230-013: Escalation branch routes to WF-900 for validation or policy failures
- TEST-PH3B-M-230-014: Output packet includes timestamp, writer_id, skill_id, instance_id, schema_version
- TEST-PH3B-M-230-015: Forbidden mutation attempt is blocked and audited
- TEST-PH3B-M-230-016: Downstream routing hints are registry-bound and deterministic
- TEST-PH3B-M-230-017: Audit entry includes operation, route_id, source packet, and confidence markers
- TEST-PH3B-M-230-018: Acceptance gate fails closed when governance metadata is incomplete
