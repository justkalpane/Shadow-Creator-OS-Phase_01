# SKL-PH3B-M-217-B_ROLL_SOURCING_STRATEGY

## 1. Skill Identity
- Skill ID: M-217
- Skill Name: B-Roll Sourcing Strategy
- Legacy Alias (Compatibility): B-Roll Requirements Mapper
- Alias Names: B-Roll Requirements Mapper
...
## 2. Purpose
Specify B-roll and supplementary footage requirements with deterministic coverage mapping.
...
- Upstream Dependencies: shot_list (from M-216), script_beat_breakdown
- Upstream Skill IDs: M-216
- Downstream Consumers: M-218 Video Editing Script
- Downstream Skill IDs: M-218
...
STEP 3: Load deterministic video policy profile for B-Roll Sourcing Strategy.
...
STEP 5: Generate primary output payload field footage_requirements.
...
STEP 8: Assemble typed output packet broll_packet.
STEP 9: Append packet to dossier.media_vein.b_roll_sourcing_strategy and append se_packet_index row.
STEP 10: Emit deterministic routing decision to M-218 or WF-900/WF-021.

## 7. Outputs
- Output Packet Family: broll_packet
- JSON Schema Reference: schemas/packets/broll_packet.schema.json
- Dossier Write Target: dossier.media_vein.b_roll_sourcing_strategy
- se_packet_index Registration: required append row with lineage and audit metadata

~~~json
{
  "instance_id": "M-217-[timestamp]-[instance]",
  "artifact_family": "broll_packet",
  ...
  "payload": {
    "skill_id": "M-217",
    "skill_name": "B-Roll Sourcing Strategy",
    "primary_output": {
      "footage_requirements": "[artifact]"
    },
    "routing": {
      "on_success": "M-218",
      "on_error": "WF-900",
...
## 12. Validation/Done
**Test Cases**
- TEST-PH3B-M-217-001: Valid required inputs produce deterministic broll_packet output
- TEST-PH3B-M-217-002: Missing dossier_id routes to WF-900 with explicit validation details
- TEST-PH3B-M-217-003: Missing route_id routes to WF-900 without packet emission
- TEST-PH3B-M-217-004: Missing required upstream packet fails closed and escalates to WF-900
- TEST-PH3B-M-217-005: Lineage references are preserved for all upstream dependencies
- TEST-PH3B-M-217-006: Primary output field footage_requirements is present and non-empty
- TEST-PH3B-M-217-007: Deterministic logic yields same output structure for identical input state
- TEST-PH3B-M-217-008: Schema validation rejects untyped payload prior to dossier mutation
- TEST-PH3B-M-217-009: Dossier writes target only dossier.media_vein.b_roll_sourcing_strategy
- TEST-PH3B-M-217-010: Mutation uses append_to_array and never overwrites prior data
- TEST-PH3B-M-217-011: se_packet_index append row includes lineage_reference and instance_id
- TEST-PH3B-M-217-012: Replay branch routes to WF-021 for remodify requests
- TEST-PH3B-M-217-013: Escalation branch routes to WF-900 for validation or policy failures
- TEST-PH3B-M-217-014: Output packet includes timestamp, writer_id, skill_id, instance_id, schema_version
- TEST-PH3B-M-217-015: Forbidden mutation attempt is blocked and audited
- TEST-PH3B-M-217-016: Downstream routing hints are registry-bound and deterministic
- TEST-PH3B-M-217-017: Audit entry includes operation, route_id, source packet, and confidence markers
- TEST-PH3B-M-217-018: Acceptance gate fails closed when governance metadata is incomplete
