# SKL-PH3B-M-226-VIDEO_SEO_OPTIMIZER

## 1. Skill Identity
- Skill ID: M-226
- Skill Name: Video SEO Optimizer
- Legacy Alias (Compatibility): Video SEO Specification
- Alias Names: Video SEO Specification
...
## 2. Purpose
Specify video SEO metadata, keyword placement, and discoverability optimization.
...
- Upstream Dependencies: script (from M-110/M-120), youtube_script (from M-122), topic (from M-001)
- Upstream Skill IDs: M-110, M-120, M-122, M-001
- Downstream Consumers: M-227 Captions and Subtitle Generator
- Downstream Skill IDs: M-227
...
STEP 3: Load deterministic video policy profile for Video SEO Optimizer.
...
STEP 5: Generate primary output payload field metadata_specifications.
...
STEP 8: Assemble typed output packet video_seo_packet.
STEP 9: Append packet to dossier.media_vein.video_seo_optimizer and append se_packet_index row.
...
~~~json
{
  "instance_id": "M-226-[timestamp]-[instance]",
  "artifact_family": "video_seo_packet",
  ...
  "payload": {
    "skill_id": "M-226",
    "skill_name": "Video SEO Optimizer",
    "primary_output": {
      "metadata_specifications": "[artifact]"
    },
    "routing": {
      "on_success": "M-227",
      "on_error": "WF-900",
...
## 12. Validation/Done
**Test Cases**
- TEST-PH3B-M-226-001: Valid required inputs produce deterministic video_seo_packet output
- TEST-PH3B-M-226-002: Missing dossier_id routes to WF-900 with explicit validation details
- TEST-PH3B-M-226-003: Missing route_id routes to WF-900 without packet emission
- TEST-PH3B-M-226-004: Missing required upstream packet fails closed and escalates to WF-900
- TEST-PH3B-M-226-005: Lineage references are preserved for all upstream dependencies
- TEST-PH3B-M-226-006: Primary output field metadata_specifications is present and non-empty
- TEST-PH3B-M-226-007: Deterministic logic yields same output structure for identical input state
- TEST-PH3B-M-226-008: Schema validation rejects untyped payload prior to dossier mutation
- TEST-PH3B-M-226-009: Dossier writes target only dossier.media_vein.video_seo_optimizer
- TEST-PH3B-M-226-010: Mutation uses append_to_array and never overwrites prior data
- TEST-PH3B-M-226-011: se_packet_index append row includes lineage_reference and instance_id
- TEST-PH3B-M-226-012: Replay branch routes to WF-021 for remodify requests
- TEST-PH3B-M-226-013: Escalation branch routes to WF-900 for validation or policy failures
- TEST-PH3B-M-226-014: Output packet includes timestamp, writer_id, skill_id, instance_id, schema_version
- TEST-PH3B-M-226-015: Forbidden mutation attempt is blocked and audited
- TEST-PH3B-M-226-016: Downstream routing hints are registry-bound and deterministic
- TEST-PH3B-M-226-017: Audit entry includes operation, route_id, source packet, and confidence markers
- TEST-PH3B-M-226-018: Acceptance gate fails closed when governance metadata is incomplete
