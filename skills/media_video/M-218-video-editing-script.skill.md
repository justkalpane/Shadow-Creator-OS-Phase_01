# SKL-PH3B-M-218-VIDEO_EDITING_SCRIPT

## 1. Skill Identity
- Skill ID: M-218
- Skill Name: Video Editing Script
- Legacy Alias (Compatibility): Editing Sequence Designer
- Alias Names: Editing Sequence Designer
...
## 2. Purpose
Design editing sequence and cut strategy with deterministic transition logic.
...
- Upstream Dependencies: shot_list (from M-216), pacing_engine_output (from M-114), emotional_arc (from M-106)
- Upstream Skill IDs: M-216, M-114, M-106
- Downstream Consumers: M-219 Color Grading Brief
- Downstream Skill IDs: M-219
...
STEP 3: Load deterministic video policy profile for Video Editing Script.
...
STEP 5: Generate primary output payload field cut_specifications.
...
STEP 8: Assemble typed output packet editing_sequence_packet.
STEP 9: Append packet to dossier.media_vein.video_editing_script and append se_packet_index row.
...
~~~json
{
  "instance_id": "M-218-[timestamp]-[instance]",
  "artifact_family": "editing_sequence_packet",
  ...
  "payload": {
    "skill_id": "M-218",
    "skill_name": "Video Editing Script",
    "primary_output": {
      "cut_specifications": "[artifact]"
    },
    "routing": {
      "on_success": "M-219",
      "on_error": "WF-900",
...
## 12. Validation/Done
**Test Cases**
- TEST-PH3B-M-218-001: Valid required inputs produce deterministic editing_sequence_packet output
- TEST-PH3B-M-218-002: Missing dossier_id routes to WF-900 with explicit validation details
- TEST-PH3B-M-218-003: Missing route_id routes to WF-900 without packet emission
- TEST-PH3B-M-218-004: Missing required upstream packet fails closed and escalates to WF-900
- TEST-PH3B-M-218-005: Lineage references are preserved for all upstream dependencies
- TEST-PH3B-M-218-006: Primary output field cut_specifications is present and non-empty
- TEST-PH3B-M-218-007: Deterministic logic yields same output structure for identical input state
- TEST-PH3B-M-218-008: Schema validation rejects untyped payload prior to dossier mutation
- TEST-PH3B-M-218-009: Dossier writes target only dossier.media_vein.video_editing_script
- TEST-PH3B-M-218-010: Mutation uses append_to_array and never overwrites prior data
- TEST-PH3B-M-218-011: se_packet_index append row includes lineage_reference and instance_id
- TEST-PH3B-M-218-012: Replay branch routes to WF-021 for remodify requests
- TEST-PH3B-M-218-013: Escalation branch routes to WF-900 for validation or policy failures
- TEST-PH3B-M-218-014: Output packet includes timestamp, writer_id, skill_id, instance_id, schema_version
- TEST-PH3B-M-218-015: Forbidden mutation attempt is blocked and audited
- TEST-PH3B-M-218-016: Downstream routing hints are registry-bound and deterministic
- TEST-PH3B-M-218-017: Audit entry includes operation, route_id, source packet, and confidence markers
- TEST-PH3B-M-218-018: Acceptance gate fails closed when governance metadata is incomplete
