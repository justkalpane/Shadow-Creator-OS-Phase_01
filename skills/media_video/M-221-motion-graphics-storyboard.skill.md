# SKL-PH3B-M-221-MOTION_GRAPHICS_STORYBOARD

## 1. Skill Identity
- Skill ID: M-221
- Skill Name: Motion Graphics Storyboard
- Legacy Alias (Compatibility): Motion Graphics Designer
- Alias Names: Motion Graphics Designer
...
## 2. Purpose
Design motion graphics and animated overlays with deterministic timing specs.
...
- Upstream Dependencies: animation_specs (from M-210), design_brief (from M-201)
- Upstream Skill IDs: M-210, M-201
- Downstream Consumers: M-222 Video Transitions Optimizer
- Downstream Skill IDs: M-222
...
STEP 3: Load deterministic video policy profile for Motion Graphics Storyboard.
...
STEP 5: Generate primary output payload field animation_designs.
...
STEP 8: Assemble typed output packet motion_graphics_packet.
STEP 9: Append packet to dossier.media_vein.motion_graphics_storyboard and append se_packet_index row.
...
~~~json
{
  "instance_id": "M-221-[timestamp]-[instance]",
  "artifact_family": "motion_graphics_packet",
  ...
  "payload": {
    "skill_id": "M-221",
    "skill_name": "Motion Graphics Storyboard",
    "primary_output": {
      "animation_designs": "[artifact]"
    },
    "routing": {
      "on_success": "M-222",
      "on_error": "WF-900",
...
## 12. Validation/Done
**Test Cases**
- TEST-PH3B-M-221-001: Valid required inputs produce deterministic motion_graphics_packet output
- TEST-PH3B-M-221-002: Missing dossier_id routes to WF-900 with explicit validation details
- TEST-PH3B-M-221-003: Missing route_id routes to WF-900 without packet emission
- TEST-PH3B-M-221-004: Missing required upstream packet fails closed and escalates to WF-900
- TEST-PH3B-M-221-005: Lineage references are preserved for all upstream dependencies
- TEST-PH3B-M-221-006: Primary output field animation_designs is present and non-empty
- TEST-PH3B-M-221-007: Deterministic logic yields same output structure for identical input state
- TEST-PH3B-M-221-008: Schema validation rejects untyped payload prior to dossier mutation
- TEST-PH3B-M-221-009: Dossier writes target only dossier.media_vein.motion_graphics_storyboard
- TEST-PH3B-M-221-010: Mutation uses append_to_array and never overwrites prior data
- TEST-PH3B-M-221-011: se_packet_index append row includes lineage_reference and instance_id
- TEST-PH3B-M-221-012: Replay branch routes to WF-021 for remodify requests
- TEST-PH3B-M-221-013: Escalation branch routes to WF-900 for validation or policy failures
- TEST-PH3B-M-221-014: Output packet includes timestamp, writer_id, skill_id, instance_id, schema_version
- TEST-PH3B-M-221-015: Forbidden mutation attempt is blocked and audited
- TEST-PH3B-M-221-016: Downstream routing hints are registry-bound and deterministic
- TEST-PH3B-M-221-017: Audit entry includes operation, route_id, source packet, and confidence markers
- TEST-PH3B-M-221-018: Acceptance gate fails closed when governance metadata is incomplete
