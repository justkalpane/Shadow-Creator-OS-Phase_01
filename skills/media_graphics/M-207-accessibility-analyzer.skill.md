# SKL-PH3A-M-207-ACCESSIBILITY_ANALYZER

## 1. Skill Identity
- Skill ID: M-207
- Skill Name: Accessibility Analyzer
- Legacy Alias (Compatibility): Accessibility Auditor (Graphics)
- Alias Names: Graphics Accessibility Auditor
...
## 2. Purpose
Audit visual design for contrast, readability, and color-blind accessibility compliance.
...
- Upstream Dependencies: color_palette (from M-202), layout (from M-204), typography (from M-203)
- Upstream Skill IDs: M-202, M-203, M-204
- Downstream Consumers: M-208 Image Asset Sourcing Advisor
- Downstream Skill IDs: M-208
...
STEP 3: Load deterministic graphics policy profile for Accessibility Analyzer.
...
STEP 5: Generate primary output payload field contrast_scores.
...
STEP 8: Assemble typed output packet accessibility_packet.
STEP 9: Append packet to dossier.media_vein.accessibility_analyzer and append se_packet_index row.
...
~~~json
{
  "instance_id": "M-207-[timestamp]-[instance]",
  "artifact_family": "accessibility_packet",
  ...
  "payload": {
    "skill_id": "M-207",
    "skill_name": "Accessibility Analyzer",
    "primary_output": {
      "contrast_scores": "[artifact]"
    },
    "routing": {
      "on_success": "M-208",
      "on_error": "WF-900",
...
## 12. Validation/Done
**Test Cases**
- TEST-PH3A-M-207-001: Valid required inputs produce deterministic accessibility_packet output
- TEST-PH3A-M-207-002: Missing dossier_id routes to WF-900 with explicit validation details
- TEST-PH3A-M-207-003: Missing route_id routes to WF-900 without packet emission
- TEST-PH3A-M-207-004: Missing required upstream packet fails closed and escalates to WF-900
- TEST-PH3A-M-207-005: Lineage references are preserved for all upstream dependencies
- TEST-PH3A-M-207-006: Primary output field contrast_scores is present and non-empty
- TEST-PH3A-M-207-007: Deterministic logic yields same output structure for identical input state
- TEST-PH3A-M-207-008: Schema validation rejects untyped payload prior to dossier mutation
- TEST-PH3A-M-207-009: Dossier writes target only dossier.media_vein.accessibility_analyzer
- TEST-PH3A-M-207-010: Mutation uses append_to_array and never overwrites prior data
- TEST-PH3A-M-207-011: se_packet_index append row includes lineage_reference and instance_id
- TEST-PH3A-M-207-012: Replay branch routes to WF-021 for remodify requests
- TEST-PH3A-M-207-013: Escalation branch routes to WF-900 for validation or policy failures
- TEST-PH3A-M-207-014: Output packet includes timestamp, writer_id, skill_id, instance_id, schema_version
- TEST-PH3A-M-207-015: Forbidden mutation attempt is blocked and audited
- TEST-PH3A-M-207-016: Downstream routing hints are registry-bound and deterministic
- TEST-PH3A-M-207-017: Audit entry includes operation, route_id, source packet, and confidence markers
- TEST-PH3A-M-207-018: Acceptance gate fails closed when governance metadata is incomplete
