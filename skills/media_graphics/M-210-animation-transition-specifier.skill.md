# SKL-PH3A-M-210-ANIMATION_TRANSITION_SPECIFIER

## 1. Skill Identity
- Skill ID: M-210
- Skill Name: Animation and Transition Specifier
- Legacy Alias (Compatibility): Animation Specification Designer
- Alias Names: Animation Specification Designer
...
## 2. Purpose
Specify animations and transitions with deterministic timing and performance constraints.
...
- Upstream Dependencies: design_brief (from M-201), layout (from M-204), data_visualizations (from M-209)
- Upstream Skill IDs: M-201, M-204, M-209
- Downstream Consumers: M-211 Mobile-First Layout Optimizer
- Downstream Skill IDs: M-211
...
STEP 3: Load deterministic graphics policy profile for Animation and Transition Specifier.
...
STEP 5: Generate primary output payload field animation_sequences.
...
STEP 8: Assemble typed output packet animation_spec_packet.
STEP 9: Append packet to dossier.media_vein.animation_transition_specifier and append se_packet_index row.
...
~~~json
{
  "instance_id": "M-210-[timestamp]-[instance]",
  "artifact_family": "animation_spec_packet",
  ...
  "payload": {
    "skill_id": "M-210",
    "skill_name": "Animation and Transition Specifier",
    "primary_output": {
      "animation_sequences": "[artifact]"
    },
    "routing": {
      "on_success": "M-211",
      "on_error": "WF-900",
...
## 12. Validation/Done
**Test Cases**
- TEST-PH3A-M-210-001: Valid required inputs produce deterministic animation_spec_packet output
- TEST-PH3A-M-210-002: Missing dossier_id routes to WF-900 with explicit validation details
- TEST-PH3A-M-210-003: Missing route_id routes to WF-900 without packet emission
- TEST-PH3A-M-210-004: Missing required upstream packet fails closed and escalates to WF-900
- TEST-PH3A-M-210-005: Lineage references are preserved for all upstream dependencies
- TEST-PH3A-M-210-006: Primary output field animation_sequences is present and non-empty
- TEST-PH3A-M-210-007: Deterministic logic yields same output structure for identical input state
- TEST-PH3A-M-210-008: Schema validation rejects untyped payload prior to dossier mutation
- TEST-PH3A-M-210-009: Dossier writes target only dossier.media_vein.animation_transition_specifier
- TEST-PH3A-M-210-010: Mutation uses append_to_array and never overwrites prior data
- TEST-PH3A-M-210-011: se_packet_index append row includes lineage_reference and instance_id
- TEST-PH3A-M-210-012: Replay branch routes to WF-021 for remodify requests
- TEST-PH3A-M-210-013: Escalation branch routes to WF-900 for validation or policy failures
- TEST-PH3A-M-210-014: Output packet includes timestamp, writer_id, skill_id, instance_id, schema_version
- TEST-PH3A-M-210-015: Forbidden mutation attempt is blocked and audited
- TEST-PH3A-M-210-016: Downstream routing hints are registry-bound and deterministic
- TEST-PH3A-M-210-017: Audit entry includes operation, route_id, source packet, and confidence markers
- TEST-PH3A-M-210-018: Acceptance gate fails closed when governance metadata is incomplete
