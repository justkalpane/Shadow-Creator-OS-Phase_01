# SKL-PH3A-M-215-BANNER_HEADER_DESIGNER

## 1. Skill Identity
- Skill ID: M-215
- Skill Name: Banner and Header Designer
- Legacy Alias (Compatibility): Banner/Header Designer
- Alias Names: Banner/Header Designer
...
## 2. Purpose
Design banners, headers, and top-level visual branding assets for downstream media production.
...
- Upstream Dependencies: design_brief (from M-201), brand_guidelines
- Upstream Skill IDs: M-201
- Downstream Consumers: Phase 3B video production entry
- Downstream Skill IDs: M-216
...
STEP 3: Load deterministic graphics policy profile for Banner and Header Designer.
...
STEP 5: Generate primary output payload field banner_designs.
...
STEP 8: Assemble typed output packet banner_packet.
STEP 9: Append packet to dossier.media_vein.banner_header_designer and append se_packet_index row.
STEP 10: Emit deterministic routing decision to M-216 or WF-900/WF-021.

## 7. Outputs
- Output Packet Family: banner_packet
- JSON Schema Reference: schemas/packets/banner_packet.schema.json
- Dossier Write Target: dossier.media_vein.banner_header_designer
- se_packet_index Registration: required append row with lineage and audit metadata

~~~json
{
  "instance_id": "M-215-[timestamp]-[instance]",
  "artifact_family": "banner_packet",
  ...
  "payload": {
    "skill_id": "M-215",
    "skill_name": "Banner and Header Designer",
    "primary_output": {
      "banner_designs": "[artifact]"
    },
    "routing": {
      "on_success": "M-216",
      "on_error": "WF-900",
...
## 12. Validation/Done
**Test Cases**
- TEST-PH3A-M-215-001: Valid required inputs produce deterministic banner_packet output
- TEST-PH3A-M-215-002: Missing dossier_id routes to WF-900 with explicit validation details
- TEST-PH3A-M-215-003: Missing route_id routes to WF-900 without packet emission
- TEST-PH3A-M-215-004: Missing required upstream packet fails closed and escalates to WF-900
- TEST-PH3A-M-215-005: Lineage references are preserved for all upstream dependencies
- TEST-PH3A-M-215-006: Primary output field banner_designs is present and non-empty
- TEST-PH3A-M-215-007: Deterministic logic yields same output structure for identical input state
- TEST-PH3A-M-215-008: Schema validation rejects untyped payload prior to dossier mutation
- TEST-PH3A-M-215-009: Dossier writes target only dossier.media_vein.banner_header_designer
- TEST-PH3A-M-215-010: Mutation uses append_to_array and never overwrites prior data
- TEST-PH3A-M-215-011: se_packet_index append row includes lineage_reference and instance_id
- TEST-PH3A-M-215-012: Replay branch routes to WF-021 for remodify requests
- TEST-PH3A-M-215-013: Escalation branch routes to WF-900 for validation or policy failures
- TEST-PH3A-M-215-014: Output packet includes timestamp, writer_id, skill_id, instance_id, schema_version
- TEST-PH3A-M-215-015: Forbidden mutation attempt is blocked and audited
- TEST-PH3A-M-215-016: Downstream routing hints are registry-bound and deterministic
- TEST-PH3A-M-215-017: Audit entry includes operation, route_id, source packet, and confidence markers
- TEST-PH3A-M-215-018: Acceptance gate fails closed when governance metadata is incomplete
