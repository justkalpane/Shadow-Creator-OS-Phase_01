# SKL-PH3A-M-204-LAYOUT_COMPOSITION_STRATEGIST

## 1. Skill Identity
- Skill ID: M-204
- Skill Name: Layout Composition Strategist
- Legacy Alias (Compatibility): Layout Grid Generator
- Alias Names: Layout Grid Generator
...
## 2. Purpose
Create layout grids and proportional systems for cross-format visual consistency.
...
- Upstream Dependencies: design_brief (from M-201), format_specs (from M-130)
- Upstream Skill IDs: M-201, M-130
- Downstream Consumers: M-205 Icon and Illustration Mapper
- Downstream Skill IDs: M-205
...
STEP 3: Load deterministic graphics policy profile for Layout Composition Strategist.
...
STEP 5: Generate primary output payload field grid_specifications.
...
STEP 8: Assemble typed output packet layout_packet.
STEP 9: Append packet to dossier.media_vein.layout_composition_strategist and append se_packet_index row.
STEP 10: Emit deterministic routing decision to M-205 or WF-900/WF-021.

## 7. Outputs
- Output Packet Family: layout_packet
- JSON Schema Reference: schemas/packets/layout_packet.schema.json
- Dossier Write Target: dossier.media_vein.layout_composition_strategist
- se_packet_index Registration: required append row with lineage and audit metadata

~~~json
{
  "instance_id": "M-204-[timestamp]-[instance]",
  "artifact_family": "layout_packet",
  ...
  "payload": {
    "skill_id": "M-204",
    "skill_name": "Layout Composition Strategist",
    "primary_output": {
      "grid_specifications": "[artifact]"
    },
    "routing": {
      "on_success": "M-205",
      "on_error": "WF-900",
...
## 12. Validation/Done
**Test Cases**
- TEST-PH3A-M-204-001: Valid required inputs produce deterministic layout_packet output
- TEST-PH3A-M-204-002: Missing dossier_id routes to WF-900 with explicit validation details
- TEST-PH3A-M-204-003: Missing route_id routes to WF-900 without packet emission
- TEST-PH3A-M-204-004: Missing required upstream packet fails closed and escalates to WF-900
- TEST-PH3A-M-204-005: Lineage references are preserved for all upstream dependencies
- TEST-PH3A-M-204-006: Primary output field grid_specifications is present and non-empty
- TEST-PH3A-M-204-007: Deterministic logic yields same output structure for identical input state
- TEST-PH3A-M-204-008: Schema validation rejects untyped payload prior to dossier mutation
- TEST-PH3A-M-204-009: Dossier writes target only dossier.media_vein.layout_composition_strategist
- TEST-PH3A-M-204-010: Mutation uses append_to_array and never overwrites prior data
- TEST-PH3A-M-204-011: se_packet_index append row includes lineage_reference and instance_id
- TEST-PH3A-M-204-012: Replay branch routes to WF-021 for remodify requests
- TEST-PH3A-M-204-013: Escalation branch routes to WF-900 for validation or policy failures
- TEST-PH3A-M-204-014: Output packet includes timestamp, writer_id, skill_id, instance_id, schema_version
- TEST-PH3A-M-204-015: Forbidden mutation attempt is blocked and audited
- TEST-PH3A-M-204-016: Downstream routing hints are registry-bound and deterministic
- TEST-PH3A-M-204-017: Audit entry includes operation, route_id, source packet, and confidence markers
- TEST-PH3A-M-204-018: Acceptance gate fails closed when governance metadata is incomplete
