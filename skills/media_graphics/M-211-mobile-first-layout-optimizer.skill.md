# SKL-PH3A-M-211-MOBILE_FIRST_LAYOUT_OPTIMIZER

## 1. Skill Identity
- Skill ID: M-211
- Skill Name: Mobile-First Layout Optimizer
- Legacy Alias (Compatibility): Mobile Optimization Engine
- Alias Names: Mobile Optimization Engine
...
## 2. Purpose
Optimize graphics for mobile-first consumption with responsive breakpoint and touch constraints.
...
- Upstream Dependencies: all graphics outputs (M-201 through M-210)
- Upstream Skill IDs: M-201, M-202, M-203, M-204, M-205, M-206, M-207, M-208, M-209, M-210
- Downstream Consumers: M-212 Dark Mode Variant Generator
- Downstream Skill IDs: M-212
...
STEP 3: Load deterministic graphics policy profile for Mobile-First Layout Optimizer.
...
STEP 5: Generate primary output payload field mobile_variants.
...
STEP 8: Assemble typed output packet mobile_optimization_packet.
STEP 9: Append packet to dossier.media_vein.mobile_first_layout_optimizer and append se_packet_index row.
...
~~~json
{
  "instance_id": "M-211-[timestamp]-[instance]",
  "artifact_family": "mobile_optimization_packet",
  ...
  "payload": {
    "skill_id": "M-211",
    "skill_name": "Mobile-First Layout Optimizer",
    "primary_output": {
      "mobile_variants": "[artifact]"
    },
    "routing": {
      "on_success": "M-212",
      "on_error": "WF-900",
...
## 12. Validation/Done
**Test Cases**
- TEST-PH3A-M-211-001: Valid required inputs produce deterministic mobile_optimization_packet output
- TEST-PH3A-M-211-002: Missing dossier_id routes to WF-900 with explicit validation details
- TEST-PH3A-M-211-003: Missing route_id routes to WF-900 without packet emission
- TEST-PH3A-M-211-004: Missing required upstream packet fails closed and escalates to WF-900
- TEST-PH3A-M-211-005: Lineage references are preserved for all upstream dependencies
- TEST-PH3A-M-211-006: Primary output field mobile_variants is present and non-empty
- TEST-PH3A-M-211-007: Deterministic logic yields same output structure for identical input state
- TEST-PH3A-M-211-008: Schema validation rejects untyped payload prior to dossier mutation
- TEST-PH3A-M-211-009: Dossier writes target only dossier.media_vein.mobile_first_layout_optimizer
- TEST-PH3A-M-211-010: Mutation uses append_to_array and never overwrites prior data
- TEST-PH3A-M-211-011: se_packet_index append row includes lineage_reference and instance_id
- TEST-PH3A-M-211-012: Replay branch routes to WF-021 for remodify requests
- TEST-PH3A-M-211-013: Escalation branch routes to WF-900 for validation or policy failures
- TEST-PH3A-M-211-014: Output packet includes timestamp, writer_id, skill_id, instance_id, schema_version
- TEST-PH3A-M-211-015: Forbidden mutation attempt is blocked and audited
- TEST-PH3A-M-211-016: Downstream routing hints are registry-bound and deterministic
- TEST-PH3A-M-211-017: Audit entry includes operation, route_id, source packet, and confidence markers
- TEST-PH3A-M-211-018: Acceptance gate fails closed when governance metadata is incomplete
