# SKL-PH3A-M-203-TYPOGRAPHY_SELECTOR

## 1. Skill Identity
- Skill ID: M-203
- Skill Name: Typography Selector
- Legacy Alias (Compatibility): Typography Selector
- Alias Names: none
...
## 2. Purpose
Select fonts and establish typography hierarchy with readability safeguards.
...
- Upstream Dependencies: design_brief (from M-201), audience_sophistication_level
- Upstream Skill IDs: M-201
- Downstream Consumers: M-204 Layout Composition Strategist
- Downstream Skill IDs: M-204
...
STEP 3: Load deterministic graphics policy profile for Typography Selector.
...
STEP 5: Generate primary output payload field font_selections.
...
STEP 8: Assemble typed output packet typography_packet.
STEP 9: Append packet to dossier.media_vein.typography_selector and append se_packet_index row.
...
~~~json
{
  "instance_id": "M-203-[timestamp]-[instance]",
  "artifact_family": "typography_packet",
  ...
  "payload": {
    "skill_id": "M-203",
    "skill_name": "Typography Selector",
    "primary_output": {
      "font_selections": "[artifact]"
    },
    "routing": {
      "on_success": "M-204",
      "on_error": "WF-900",
...
## 12. Validation/Done
**Test Cases**
- TEST-PH3A-M-203-001: Valid required inputs produce deterministic typography_packet output
- TEST-PH3A-M-203-002: Missing dossier_id routes to WF-900 with explicit validation details
- TEST-PH3A-M-203-003: Missing route_id routes to WF-900 without packet emission
- TEST-PH3A-M-203-004: Missing required upstream packet fails closed and escalates to WF-900
- TEST-PH3A-M-203-005: Lineage references are preserved for all upstream dependencies
- TEST-PH3A-M-203-006: Primary output field font_selections is present and non-empty
- TEST-PH3A-M-203-007: Deterministic logic yields same output structure for identical input state
- TEST-PH3A-M-203-008: Schema validation rejects untyped payload prior to dossier mutation
- TEST-PH3A-M-203-009: Dossier writes target only dossier.media_vein.typography_selector
- TEST-PH3A-M-203-010: Mutation uses append_to_array and never overwrites prior data
- TEST-PH3A-M-203-011: se_packet_index append row includes lineage_reference and instance_id
- TEST-PH3A-M-203-012: Replay branch routes to WF-021 for remodify requests
- TEST-PH3A-M-203-013: Escalation branch routes to WF-900 for validation or policy failures
- TEST-PH3A-M-203-014: Output packet includes timestamp, writer_id, skill_id, instance_id, schema_version
- TEST-PH3A-M-203-015: Forbidden mutation attempt is blocked and audited
- TEST-PH3A-M-203-016: Downstream routing hints are registry-bound and deterministic
- TEST-PH3A-M-203-017: Audit entry includes operation, route_id, source packet, and confidence markers
- TEST-PH3A-M-203-018: Acceptance gate fails closed when governance metadata is incomplete
