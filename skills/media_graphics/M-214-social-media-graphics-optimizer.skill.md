# SKL-PH3A-M-214-SOCIAL_MEDIA_GRAPHICS_OPTIMIZER

## 1. Skill Identity
- Skill ID: M-214
- Skill Name: Social Media Graphics Optimizer
- Legacy Alias (Compatibility): Social Media Asset Generator
- Alias Names: Social Media Asset Generator
...
## 2. Purpose
Generate platform-specific social media graphics with deterministic dimension matrices.
...
- Upstream Dependencies: design_brief (from M-201), social_captions (from M-128)
- Upstream Skill IDs: M-201, M-128
- Downstream Consumers: M-215 Banner and Header Designer
- Downstream Skill IDs: M-215
...
STEP 3: Load deterministic graphics policy profile for Social Media Graphics Optimizer.
...
STEP 5: Generate primary output payload field platform_specific_dimensions.
...
STEP 8: Assemble typed output packet social_asset_packet.
STEP 9: Append packet to dossier.media_vein.social_media_graphics_optimizer and append se_packet_index row.
STEP 10: Emit deterministic routing decision to M-215 or WF-900/WF-021.

## 7. Outputs
- Output Packet Family: social_asset_packet
- JSON Schema Reference: schemas/packets/social_asset_packet.schema.json
- Dossier Write Target: dossier.media_vein.social_media_graphics_optimizer
- se_packet_index Registration: required append row with lineage and audit metadata

~~~json
{
  "instance_id": "M-214-[timestamp]-[instance]",
  "artifact_family": "social_asset_packet",
  ...
  "payload": {
    "skill_id": "M-214",
    "skill_name": "Social Media Graphics Optimizer",
    "primary_output": {
      "platform_specific_dimensions": "[artifact]"
    },
    "routing": {
      "on_success": "M-215",
      "on_error": "WF-900",
...
## 12. Validation/Done
**Test Cases**
- TEST-PH3A-M-214-001: Valid required inputs produce deterministic social_asset_packet output
- TEST-PH3A-M-214-002: Missing dossier_id routes to WF-900 with explicit validation details
- TEST-PH3A-M-214-003: Missing route_id routes to WF-900 without packet emission
- TEST-PH3A-M-214-004: Missing required upstream packet fails closed and escalates to WF-900
- TEST-PH3A-M-214-005: Lineage references are preserved for all upstream dependencies
- TEST-PH3A-M-214-006: Primary output field platform_specific_dimensions is present and non-empty
- TEST-PH3A-M-214-007: Deterministic logic yields same output structure for identical input state
- TEST-PH3A-M-214-008: Schema validation rejects untyped payload prior to dossier mutation
- TEST-PH3A-M-214-009: Dossier writes target only dossier.media_vein.social_media_graphics_optimizer
- TEST-PH3A-M-214-010: Mutation uses append_to_array and never overwrites prior data
- TEST-PH3A-M-214-011: se_packet_index append row includes lineage_reference and instance_id
- TEST-PH3A-M-214-012: Replay branch routes to WF-021 for remodify requests
- TEST-PH3A-M-214-013: Escalation branch routes to WF-900 for validation or policy failures
- TEST-PH3A-M-214-014: Output packet includes timestamp, writer_id, skill_id, instance_id, schema_version
- TEST-PH3A-M-214-015: Forbidden mutation attempt is blocked and audited
- TEST-PH3A-M-214-016: Downstream routing hints are registry-bound and deterministic
- TEST-PH3A-M-214-017: Audit entry includes operation, route_id, source packet, and confidence markers
- TEST-PH3A-M-214-018: Acceptance gate fails closed when governance metadata is incomplete
