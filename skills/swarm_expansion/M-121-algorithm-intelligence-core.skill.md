# SKL-PH2C-M-121-TIKTOK_SCRIPT_OPTIMIZER

## 1. Skill Identity
- Skill ID: M-121
- Skill Name: TikTok Script Optimizer
- Legacy Alias (Filename Compatibility): Algorithm Intelligence Core
...
## 2. Purpose
Compress script to 15-30 second TikTok format with visual pacing markers and sound-on/deaf-viewing optimization.
...
- Platform Specifics: 15-30 second vertical format, high-speed pacing, strong visual cueing, sound-on plus caption-safe rendering

## 4. Workflow Injection
- Producer Workflow: CWF-230
- Consumer Workflows: CWF-240
- Upstream Dependencies: base_script (from M-110/M-120), creator_voice_profile, platform_constraints.tiktok
...
STEP 3: Load deterministic platform contract for TikTok Script Optimizer.
...
STEP 8: Assemble typed output packet tiktok_script_packet and validate against JSON schema.
STEP 9: Append packet to dossier.script_vein.tiktok_script_optimizer and append lineage row to se_packet_index.
STEP 10: Emit routing decision (success, replay, escalate) with deterministic next-hop references.

## 7. Outputs
- Output Packet Family: tiktok_script_packet
- JSON Schema Reference: schemas/packets/tiktok_script_packet.schema.json
- Dossier Write Target: dossier.script_vein.tiktok_script_optimizer
- se_packet_index Registration: required append row with lineage and audit metadata

~~~json
{
  "instance_id": "M-121-[timestamp]-[instance]",
  "artifact_family": "tiktok_script_packet",
  ...
  "payload": {
    "skill_id": "M-121",
    "skill_name": "TikTok Script Optimizer",
    "platform": "TikTok",
    "primary_output": {
      "compressed_script": "[platform-adapted artifact]"
    },
    "variants": [
...
- Never mutate dossier fields outside dossier.script_vein.tiktok_script_optimizer.
...
## 12. Validation/Done
**Test Cases**
- TEST-PH2C-M-121-001: Valid required inputs produce deterministic tiktok_script_packet output
- TEST-PH2C-M-121-002: Missing dossier_id routes to WF-900 with explicit validation error
- TEST-PH2C-M-121-003: Missing base input payload routes to WF-900 without packet emission
- TEST-PH2C-M-121-004: Unsupported route_id resolves via deterministic fallback and logs route correction
- TEST-PH2C-M-121-005: Platform constraints are enforced without random branch selection
- TEST-PH2C-M-121-006: Primary output field compressed_script is present and non-empty
- TEST-PH2C-M-121-007: Variant generation creates exactly 3 bounded variants when requested
- TEST-PH2C-M-121-008: Governance safety check blocks forbidden claims and routes to WF-900
- TEST-PH2C-M-121-009: Replay request envelope routes to WF-021 with replay-ready context
- TEST-PH2C-M-121-010: Schema validation rejects untyped payload before dossier mutation
- TEST-PH2C-M-121-011: Dossier write target uses dossier.script_vein.tiktok_script_optimizer only
- TEST-PH2C-M-121-012: Mutation uses append_to_array and never overwrites prior packet history
- TEST-PH2C-M-121-013: se_packet_index row is appended with lineage_reference and instance_id
- TEST-PH2C-M-121-014: Output packet includes timestamp, writer_id, skill_id, instance_id, schema_version
- TEST-PH2C-M-121-015: Downstream routing hint is deterministic for identical input state
- TEST-PH2C-M-121-016: Forbidden mutation attempt is rejected and escalated to WF-900
- TEST-PH2C-M-121-017: Audit entry captures operation, route_id, source packet, and confidence markers
- TEST-PH2C-M-121-018: Acceptance gate fails closed when governance metadata is incomplete
