# SKL-PH1-RETENTION-DROP-DETECTOR

## 1. Skill Identity
- **Skill ID:** M-125
- **Skill Name:** Retention Drop Detector
- **Version:** 1.0.0
...
## 2. Purpose
Replica skill artifact for M-125 (Retention Drop Detector), generated using the latest repository skill DNA template and runtime contract. This skill preserves the same structure, routing semantics, escalation behavior, and mutation law as the current compliant skills.

## 3. DNA Injection
- **Role Definition:** retention-drop-detector_executor
- **DNA Archetype:** Indra
...
- **Downstream Handoff:** retention-drop-detector_packet -> downstream workflow chain
...
3. Execute core transformation logic for M-125
...
```
**Write Targets:**
- dossier.expansion_vein.retention-drop-detector (append_to_array)
...
**Writes:**
- dossier.expansion_vein.retention-drop-detector (append_only)
...
**Acceptance Tests:**
- TEST-PH1-M125-001: valid input produces CREATED packet
- TEST-PH1-M125-002: invalid input escalates to WF-900
- TEST-PH1-M125-003: dossier patch is additive only
