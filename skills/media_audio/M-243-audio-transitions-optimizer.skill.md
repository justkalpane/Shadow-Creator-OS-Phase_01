# SKL-PH3C-M-243-AUDIO_TRANSITIONS_OPTIMIZER

## 1. Skill Identity
- Skill ID: M-243
- Skill Name: Audio Transitions Optimizer
- Legacy Alias (Compatibility): Transcript & Captions Generator
- Alias Names: Transcript & Captions Generator
...
- Upstream Dependencies: voiceover_script (from M-110/M-124), audio_file
- Upstream Skill IDs: M-110, M-124
- Downstream Consumers: M-244 Transcript Generator
- Downstream Skill IDs: M-244
...
STEP 3: Load deterministic audio policy profile for Audio Transitions Optimizer.
...
STEP 5: Generate primary output payload field transcript_text.
...
STEP 8: Assemble typed output packet transcript_packet.
STEP 9: Append packet to dossier.media_vein.audio_transitions_optimizer and append se_packet_index row.
STEP 10: Emit deterministic routing decision to M-244 or WF-900/WF-021.

## 7. Outputs
- Output Packet Family: transcript_packet
- JSON Schema Reference: schemas/packets/transcript_packet.schema.json
- Dossier Write Target: dossier.media_vein.audio_transitions_optimizer
- se_packet_index Registration: required append row with lineage and audit metadata

~~~json
{
  "instance_id": "M-243-[timestamp]-[instance]",
  "artifact_family": "transcript_packet",
  ...
  "payload": {
    "skill_id": "M-243",
    "skill_name": "Audio Transitions Optimizer",
    "primary_output": {
      "transcript_text": "[artifact]"
    },
    "routing": {
      "on_success": "M-244",
      "on_error": "WF-900",
...
## 12. Validation/Done
**Test Cases**
- TEST-PH3C-M-243-001: Valid required inputs produce deterministic transcript_packet output
- TEST-PH3C-M-243-002: Missing dossier_id routes to WF-900 with explicit validation details
- TEST-PH3C-M-243-003: Missing route_id routes to WF-900 without packet emission
- TEST-PH3C-M-243-004: Missing required upstream packet fails closed and escalates to WF-900
- TEST-PH3C-M-243-005: Lineage references are preserved for all upstream dependencies
- TEST-PH3C-M-243-006: Primary output field transcript_text is present and non-empty
- TEST-PH3C-M-243-007: Deterministic logic yields same output structure for identical input state
- TEST-PH3C-M-243-008: Schema validation rejects untyped payload prior to dossier mutation
- TEST-PH3C-M-243-009: Dossier writes target only dossier.media_vein.audio_transitions_optimizer
- TEST-PH3C-M-243-010: Mutation uses append_to_array and never overwrites prior data
- TEST-PH3C-M-243-011: se_packet_index append row includes lineage_reference and instance_id
- TEST-PH3C-M-243-012: Replay branch routes to WF-021 for remodify requests
- TEST-PH3C-M-243-013: Escalation branch routes to WF-900 for validation or policy failures
- TEST-PH3C-M-243-014: Output packet includes timestamp, writer_id, skill_id, instance_id, schema_version
- TEST-PH3C-M-243-015: Forbidden mutation attempt is blocked and audited
- TEST-PH3C-M-243-016: Downstream routing hints are registry-bound and deterministic
- TEST-PH3C-M-243-017: Audit entry includes operation, route_id, source packet, and confidence markers
- TEST-PH3C-M-243-018: Acceptance gate fails closed when governance metadata is incomplete
