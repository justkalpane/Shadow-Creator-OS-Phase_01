# SKL-PH3C-M-237-AUDIO_LEVELS_NORMALIZATION

## 1. Skill Identity
- Skill ID: M-237
- Skill Name: Audio Levels and Normalization
- Legacy Alias (Compatibility): Levels & Dynamics Optimizer
- Alias Names: Levels & Dynamics Optimizer
...
- Upstream Dependencies: voiceover_spec (from M-231), music (from M-233), sound_design (from M-232)
- Upstream Skill IDs: M-231, M-233, M-232
- Downstream Consumers: M-238 Compression and EQ Specifications
- Downstream Skill IDs: M-238
...
STEP 3: Load deterministic audio policy profile for Audio Levels and Normalization.
...
STEP 5: Generate primary output payload field target_levels.
...
STEP 8: Assemble typed output packet levels_packet.
STEP 9: Append packet to dossier.media_vein.audio_levels_normalization and append se_packet_index row.
STEP 10: Emit deterministic routing decision to M-238 or WF-900/WF-021.

## 7. Outputs
- Output Packet Family: levels_packet
- JSON Schema Reference: schemas/packets/levels_packet.schema.json
- Dossier Write Target: dossier.media_vein.audio_levels_normalization
- se_packet_index Registration: required append row with lineage and audit metadata

~~~json
{
  "instance_id": "M-237-[timestamp]-[instance]",
  "artifact_family": "levels_packet",
  ...
  "payload": {
    "skill_id": "M-237",
    "skill_name": "Audio Levels and Normalization",
    "primary_output": {
      "target_levels": "[artifact]"
    },
    "routing": {
      "on_success": "M-238",
      "on_error": "WF-900",
...
## 12. Validation/Done
**Test Cases**
- TEST-PH3C-M-237-001: Valid required inputs produce deterministic levels_packet output
- TEST-PH3C-M-237-002: Missing dossier_id routes to WF-900 with explicit validation details
- TEST-PH3C-M-237-003: Missing route_id routes to WF-900 without packet emission
- TEST-PH3C-M-237-004: Missing required upstream packet fails closed and escalates to WF-900
- TEST-PH3C-M-237-005: Lineage references are preserved for all upstream dependencies
- TEST-PH3C-M-237-006: Primary output field target_levels is present and non-empty
- TEST-PH3C-M-237-007: Deterministic logic yields same output structure for identical input state
- TEST-PH3C-M-237-008: Schema validation rejects untyped payload prior to dossier mutation
- TEST-PH3C-M-237-009: Dossier writes target only dossier.media_vein.audio_levels_normalization
- TEST-PH3C-M-237-010: Mutation uses append_to_array and never overwrites prior data
- TEST-PH3C-M-237-011: se_packet_index append row includes lineage_reference and instance_id
- TEST-PH3C-M-237-012: Replay branch routes to WF-021 for remodify requests
- TEST-PH3C-M-237-013: Escalation branch routes to WF-900 for validation or policy failures
- TEST-PH3C-M-237-014: Output packet includes timestamp, writer_id, skill_id, instance_id, schema_version
- TEST-PH3C-M-237-015: Forbidden mutation attempt is blocked and audited
- TEST-PH3C-M-237-016: Downstream routing hints are registry-bound and deterministic
- TEST-PH3C-M-237-017: Audit entry includes operation, route_id, source packet, and confidence markers
- TEST-PH3C-M-237-018: Acceptance gate fails closed when governance metadata is incomplete
