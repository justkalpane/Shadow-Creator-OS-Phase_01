# SKL-PHASE_1C_CONDITIONAL_RESEARCH-M-022-LONGITUDINAL_ANALYSIS_ENGINE

## 1. Skill Identity
- Skill ID: M-022
- Skill Name: Longitudinal Analysis Engine
- Legacy Alias (Filename Compatibility): Knowledge Graph Builder
...
## 2. Purpose
Identify temporal patterns, trend evolution, and lag structures over time.

## 3. DNA Injection
- Archetype: Shiva (transformer, change analyzer)
...
- Upstream Dependencies: research_narrative (from M-018), historical_data_sources
- Upstream Skill IDs: M-018
- Downstream Consumers: M-023 Causal Inference Builder
- Downstream Skill IDs: M-023
...
STEP 4: Build deterministic analysis frame for Longitudinal Analysis Engine.
...
STEP 5: Generate primary output payload field temporal_trends.
...
STEP 8: Validate typed packet against schemas/packets/longitudinal_packet.schema.json.
STEP 9: Append packet to dossier.research_vein.longitudinal_analysis_engine and append se_packet_index lineage row.
...
~~~json
{
  "instance_id": "M-022-[timestamp]-[instance]",
  "artifact_family": "longitudinal_packet",
  ...
  "payload": {
    "skill_id": "M-022",
    "skill_name": "Longitudinal Analysis Engine",
    "execution_condition": "m020_confidence_score_lt_0_85",
    "primary_output": {
      "temporal_trends": "[artifact]"
    },
    "routing": {
      "on_success": "M-023",
      "on_error": "WF-900",
...
## 12. Validation/Done
**Test Cases**
- TEST-PH1C-M-022-001: Valid required inputs produce deterministic longitudinal_packet output
- TEST-PH1C-M-022-002: Missing dossier_id routes to WF-900 with explicit validation details
- TEST-PH1C-M-022-003: Missing route_id routes to WF-900 without packet emission
- TEST-PH1C-M-022-004: Missing required upstream packet fails closed and escalates to WF-900
- TEST-PH1C-M-022-005: Lineage references are preserved for all upstream dependencies
- TEST-PH1C-M-022-006: Primary output field temporal_trends is present and non-empty
- TEST-PH1C-M-022-007: Deterministic logic yields same output structure for identical input state
- TEST-PH1C-M-022-008: Schema validation rejects untyped payload prior to dossier mutation
- TEST-PH1C-M-022-009: Dossier writes target only dossier.research_vein.longitudinal_analysis_engine
- TEST-PH1C-M-022-010: Mutation uses append_to_array and never overwrites prior data
- TEST-PH1C-M-022-011: se_packet_index append row includes lineage_reference and instance_id
- TEST-PH1C-M-022-012: Replay branch routes to WF-021 for remodify requests
- TEST-PH1C-M-022-013: Escalation branch routes to WF-900 for validation or policy failures
- TEST-PH1C-M-022-014: Output packet includes timestamp, writer_id, skill_id, instance_id, schema_version
- TEST-PH1C-M-022-015: Forbidden mutation attempt is blocked and audited
- TEST-PH1C-M-022-016: Downstream routing hints are registry-bound and deterministic
- TEST-PH1C-M-022-017: Audit entry includes operation, route_id, source packet, and confidence markers
- TEST-PH1C-M-022-018: Acceptance gate fails closed when governance metadata is incomplete
