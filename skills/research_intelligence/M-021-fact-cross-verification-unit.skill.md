# SKL-PHASE_1C_CONDITIONAL_RESEARCH-M-021-PRIMARY_SOURCE_INTERROGATOR

## 1. Skill Identity
- Skill ID: M-021
- Skill Name: Primary Source Interrogator
- Legacy Alias (Filename Compatibility): Fact Cross-Verification Unit
...
## 2. Purpose
Deep interrogation of primary sources to extract latent claims and assumptions.

## 3. DNA Injection
- Archetype: Saraswati (knowledge keeper)
...
- Upstream Dependencies: verified_claims (from M-015), expert_opinions (from M-017)
- Upstream Skill IDs: M-015, M-017
- Downstream Consumers: M-022 Longitudinal Analysis Engine
- Downstream Skill IDs: M-022
...
STEP 4: Build deterministic analysis frame for Primary Source Interrogator.
...
STEP 5: Generate primary output payload field extracted_assumptions.
...
STEP 8: Validate typed packet against schemas/packets/primary_source_interrogation_packet.schema.json.
STEP 9: Append packet to dossier.research_vein.primary_source_interrogator and append se_packet_index lineage row.
...
~~~json
{
  "instance_id": "M-021-[timestamp]-[instance]",
  "artifact_family": "primary_source_interrogation_packet",
  ...
  "payload": {
    "skill_id": "M-021",
    "skill_name": "Primary Source Interrogator",
    "execution_condition": "m020_confidence_score_lt_0_85",
    "primary_output": {
      "extracted_assumptions": "[artifact]"
    },
    "routing": {
      "on_success": "M-022",
      "on_error": "WF-900",
...
## 12. Validation/Done
**Test Cases**
- TEST-PH1C-M-021-001: Valid required inputs produce deterministic primary_source_interrogation_packet output
- TEST-PH1C-M-021-002: Missing dossier_id routes to WF-900 with explicit validation details
- TEST-PH1C-M-021-003: Missing route_id routes to WF-900 without packet emission
- TEST-PH1C-M-021-004: Missing required upstream packet fails closed and escalates to WF-900
- TEST-PH1C-M-021-005: Lineage references are preserved for all upstream dependencies
- TEST-PH1C-M-021-006: Primary output field extracted_assumptions is present and non-empty
- TEST-PH1C-M-021-007: Deterministic logic yields same output structure for identical input state
- TEST-PH1C-M-021-008: Schema validation rejects untyped payload prior to dossier mutation
- TEST-PH1C-M-021-009: Dossier writes target only dossier.research_vein.primary_source_interrogator
- TEST-PH1C-M-021-010: Mutation uses append_to_array and never overwrites prior data
- TEST-PH1C-M-021-011: se_packet_index append row includes lineage_reference and instance_id
- TEST-PH1C-M-021-012: Replay branch routes to WF-021 for remodify requests
- TEST-PH1C-M-021-013: Escalation branch routes to WF-900 for validation or policy failures
- TEST-PH1C-M-021-014: Output packet includes timestamp, writer_id, skill_id, instance_id, schema_version
- TEST-PH1C-M-021-015: Forbidden mutation attempt is blocked and audited
- TEST-PH1C-M-021-016: Downstream routing hints are registry-bound and deterministic
- TEST-PH1C-M-021-017: Audit entry includes operation, route_id, source packet, and confidence markers
- TEST-PH1C-M-021-018: Acceptance gate fails closed when governance metadata is incomplete
