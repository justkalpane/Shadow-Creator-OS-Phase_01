# SKL-PHASE_1C_CONDITIONAL_RESEARCH-M-023-CAUSAL_INFERENCE_BUILDER

## 1. Skill Identity
- Skill ID: M-023
- Skill Name: Causal Inference Builder
- Legacy Alias (Filename Compatibility): Trend Risk Evaluator
...
## 2. Purpose
Construct formal causal models from correlational evidence and confounder checks.

## 3. DNA Injection
- Archetype: Brihaspati (wisdom keeper, logical analyzer)
...
- Upstream Dependencies: verified_claims (from M-015), counterarguments (from M-016), longitudinal_analysis (from M-022)
- Upstream Skill IDs: M-015, M-016, M-022
- Downstream Consumers: M-024 Meta-Analysis Synthesizer
- Downstream Skill IDs: M-024
...
STEP 4: Build deterministic analysis frame for Causal Inference Builder.
...
STEP 5: Generate primary output payload field causal_models.
...
STEP 8: Validate typed packet against schemas/packets/causal_inference_packet.schema.json.
STEP 9: Append packet to dossier.research_vein.causal_inference_builder and append se_packet_index lineage row.
...
~~~json
{
  "instance_id": "M-023-[timestamp]-[instance]",
  "artifact_family": "causal_inference_packet",
  ...
  "payload": {
    "skill_id": "M-023",
    "skill_name": "Causal Inference Builder",
    "execution_condition": "m020_confidence_score_lt_0_85",
    "primary_output": {
      "causal_models": "[artifact]"
    },
    "routing": {
      "on_success": "M-024",
      "on_error": "WF-900",
...
## 12. Validation/Done
**Test Cases**
- TEST-PH1C-M-023-001: Valid required inputs produce deterministic causal_inference_packet output
- TEST-PH1C-M-023-002: Missing dossier_id routes to WF-900 with explicit validation details
- TEST-PH1C-M-023-003: Missing route_id routes to WF-900 without packet emission
- TEST-PH1C-M-023-004: Missing required upstream packet fails closed and escalates to WF-900
- TEST-PH1C-M-023-005: Lineage references are preserved for all upstream dependencies
- TEST-PH1C-M-023-006: Primary output field causal_models is present and non-empty
- TEST-PH1C-M-023-007: Deterministic logic yields same output structure for identical input state
- TEST-PH1C-M-023-008: Schema validation rejects untyped payload prior to dossier mutation
- TEST-PH1C-M-023-009: Dossier writes target only dossier.research_vein.causal_inference_builder
- TEST-PH1C-M-023-010: Mutation uses append_to_array and never overwrites prior data
- TEST-PH1C-M-023-011: se_packet_index append row includes lineage_reference and instance_id
- TEST-PH1C-M-023-012: Replay branch routes to WF-021 for remodify requests
- TEST-PH1C-M-023-013: Escalation branch routes to WF-900 for validation or policy failures
- TEST-PH1C-M-023-014: Output packet includes timestamp, writer_id, skill_id, instance_id, schema_version
- TEST-PH1C-M-023-015: Forbidden mutation attempt is blocked and audited
- TEST-PH1C-M-023-016: Downstream routing hints are registry-bound and deterministic
- TEST-PH1C-M-023-017: Audit entry includes operation, route_id, source packet, and confidence markers
- TEST-PH1C-M-023-018: Acceptance gate fails closed when governance metadata is incomplete
