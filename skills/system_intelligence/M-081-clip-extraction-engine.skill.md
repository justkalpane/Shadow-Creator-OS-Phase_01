# SKL-PH1-CLIP-EXTRACTION-ENGINE
...
## 2. Purpose
Skill artifact for M-081 (Clip Extraction Engine), generated using the latest repository skill DNA template and runtime contract. This skill preserves the same structure, routing semantics, escalation behavior, and mutation law as the current compliant skills.

## 3. DNA Injection
- **Role Definition:** clip-extraction-engine_executor
- **DNA Archetype:** Aruna
...
- **Downstream Handoff:** clip-extraction-engine_packet -> downstream workflow chain
...
3. Execute core transformation logic for M-081
...
```json
{
  "instance_id": "M-081-[timestamp]",
  "artifact_family": "clip-extraction-engine_packet",
  ...
  "payload": {
    "skill_id": "M-081",
    "skill_name": "Clip Extraction Engine",
    "result": {}
  }
}
```
**Write Targets:**
- dossier.runtime_vein.clip-extraction-engine (append_to_array)
...
**Writes:**
- dossier.runtime_vein.clip-extraction-engine (append_only)
...
**Acceptance Tests:**
- TEST-PH1-M081-001: valid input produces CREATED packet
- TEST-PH1-M081-002: invalid input escalates to WF-900
- TEST-PH1-M081-003: dossier patch is additive only
